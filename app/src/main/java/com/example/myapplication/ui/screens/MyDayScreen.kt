    ...
    Column(modifier = Modifier.fillMaxSize().padding(horizontal = 20.dp)) {
        ...
        if (tasks.isEmpty()) {
            Column(Modifier.fillMaxSize(), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
                ...
                Text("Hôm nay sạch sẽ!", fontSize = 20.sp, fontWeight = FontWeight.Medium, color = NeumorphicColors.textPrimary)
            }
        } else {
            LazyColumn(verticalArrangement = Arrangement.spacedBy(16.dp), modifier = Modifier.fillMaxWidth()) {
                items(tasks, key = { it.id }) { task ->
                    TaskCard(task = task, onToggle = { onTaskToggle(task.id) }, onDelete = { onTaskDelete(task.id) })
                }
            }
        }
    }
